...
class Parser():
  ...
  def __init__(self,
    ...
    subset_indices = torch.randperm(len(self.valid_dataset))[:100]
    self.calib_dataset = Subset(self.valid_dataset, subset_indices)

    self.calibloader = torch.utils.data.DataLoader(self.calib_dataset,
                                                   batch_size=self.batch_size,
                                                   shuffle=False,
                                                   num_workers=self.workers,
                                                   pin_memory=True,
                                                   drop_last=True)
    assert len(self.calibloader) > 0
    self.calibiter = iter(self.calibloader)
